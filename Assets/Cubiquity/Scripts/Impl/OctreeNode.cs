                        ...
                        if (cuOctreeNode.hasMesh == 1)
                        {
                            ...
                            if (volumeCollider != null)
                            {
                                ...
                                if (useCollider)
                                {
                                    ...
                                    /*
                                    string name2Dcollider = nodeGameObject.name + "2DCollider";
                                    Transform collider2Dholder = nodeGameObject.transform.FindChild(name2Dcollider);
                                    if(collider2Dholder == null) {
                                        collider2Dholder = new GameObject(name2Dcollider, typeof(PolygonCollider2D)).transform;
                                        collider2Dholder.SetParent(nodeGameObject.transform);
                                        collider2Dholder.localPosition =new Vector3(0,0, -nodeGameObject.transform.position.z);

                                    }

                                    MeshConversion.Update2DColliderForNode(collider2Dholder.GetComponent<PolygonCollider2D>(), -nodeGameObject.transform.position.z +0.5f, 0.5f, collisionMesh);
                                    */
                                }
                            }
                        }
