...
namespace Cubiquity
{
    namespace Impl
    {
        public class MeshConversion
        {
            ...
            static bool IsInsidePlane(Vector3 vert, float depth, float epsilon) {
                return vert.z > depth - epsilon && vert.z < depth + epsilon;
            }
            
            //TODO: Move to own class
            public static void Update2DColliderForNode(PolygonCollider2D collider, float depth, float epsilon, Mesh mesh) {
                int maxPaths = int.MaxValue;
                //Make sure that bounds cross out plane
                if (mesh.bounds.max.z < depth - epsilon)
                    return;
                if (mesh.bounds.min.z > depth + epsilon)
                    return;
                ...
                for (int i = 0; i < triangles.Length; i += 3) {
                    ...
                    for (int corner = 0; corner < 3; corner++) {
                        Vector3 point = vertices[triangles[i + corner]];
                        if (IsInsidePlane(point, depth, epsilon)) {
                            numInside++;
                        }
                    }
                    if (numInside > 2)
                ...
                noOfPaths = maxPaths < noOfPaths ? maxPaths : noOfPaths;
                collider.pathCount = noOfPaths;
                int currentPath = 0;
                
                Vector2[] fullTri = new Vector2[3];
                for (int i = 0; i < triangles.Length; i+=3) {
                    int numInside = 0;
                    for (int corner = 0; corner < 3; corner++) {
                        Vector3 point = vertices[triangles[i + corner]];
                        if(IsInsidePlane(point, depth, epsilon)) {
                            ...
                            fullTri[numInside] = point;
                            numInside++;
                        }
                    }
                    if(numInside == 3) {
                        //Someone on the internet said it should be faster to do reverse order
                        collider.SetPath(noOfPaths - currentPath-1, fullTri);
                        currentPath++;
                    }
                    if (currentPath >= noOfPaths) return;
                }
